...
SELECT 
    pizza_types.name, pizzas.price AS high_price
FROM
    pizza_types
        JOIN
...
-- Q6 Join the neccessary tables to find the total quantity of each pizza category ordered.
SELECT 
...
-- Q12 Analyze the cumilative revenue generated over time.
select order_date, sum(revenue) over(order by order_date) as cumilative_revenue from
(select orders.order_date, sum(order_details.quantity * pizzas.price) as revenue from orders join order_details 
on orders.order_id = order_details.order_id join pizzas on order_details.pizza_id = pizzas.pizza_id 
...
group by pizza_types.category, pizza_types.name) as a) as b where rk <= 3;
